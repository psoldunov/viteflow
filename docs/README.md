...
Reusable Bun + Vite template for building Webflow custom JavaScript with live reload, source maps, and production bundling.
...
5. [Styles](./styles.md) — CSS imports, dev reloads, production inlining
6. [Development](./development.md) — how the proxy works, reload behavior, debugging
...
Browser opens `http://localhost:5173/`. You see your Webflow staging site with localhost JS injected. Edit any `/src` file, and the browser reloads with your changes.

## What you get

- File-based routing for `window.location.pathname` (Next.js style)
- Full browser reloads on TypeScript, TSX, and CSS edits
- Source maps in dev so console errors point at original `.ts` and `.tsx` lines
