...
Viteflow scans `/src/**/*.ts` and `/src/**/*.tsx` at startup and builds a route table. At runtime, the router matches `window.location.pathname` against the table and runs the matching handler.

## Filename rules

| File path | URL pathname |
|-----------|--------------|
| `src/_global.ts` or `.tsx` | runs on every page (not a route — see below) |
| `src/index.ts` or `.tsx` | `/` |
| `src/about.ts` or `.tsx` | `/about` |
| `src/blog.ts` or `.tsx` | `/blog` |
| `src/blog/index.ts` or `.tsx` | `/blog` (collides with `src/blog.ts` — error) |
| `src/blog/[slug].ts` or `.tsx` | `/blog/:slug` (any segment after `/blog/`) |
| `src/shop/cart.ts` or `.tsx` | `/shop/cart` |
| `src/shop/[id].ts` or `.tsx` | `/shop/:id` |
| `src/shop/[id]/edit.ts` or `.tsx` | `/shop/:id/edit` |

### Conversion algorithm

1. Strip the `/src` prefix and the `.ts` or `.tsx` suffix.
...
src/users/[id]/posts/[postId].tsx → /users/:id/posts/:postId
```
...
`src/_global.ts` and `src/_global.tsx` are special. The global file is not a route — it runs on **every page** before the matched route handler. The leading underscore signals that it is not a route file (matching the same convention as `_lib/` private utilities).
...
If neither global file exists, viteflow skips it silently. If both exist, viteflow throws a duplicate global handler error.

## What is not supported

- **Catch-all routes.** No `[...rest].ts` or `[...rest].tsx`. Route precisely or match a parent literal segment and inspect `window.location.pathname` yourself.
...
Just create the file. Vite picks up the new route on the next save. The router rebuilds, your browser reloads, and the route is live.
